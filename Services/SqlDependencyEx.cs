    ...
    public sealed class SqlDependencyEx : IDisposable
    {
        public class TableChangedEventArgs : EventArgs
        {
            ...
            public XElement Data
            {
                get
                {
                    if (string.IsNullOrWhiteSpace(_notificationMessage)) return null;

                    return ReadXDocumentWithInvalidCharacters(_notificationMessage);
                }
            }

            public NotificationTypeEnum NotificationType =>
                Data?.Element(InsertedTag) != null
                    ? Data?.Element(DeletedTag) != null
                        ? NotificationTypeEnum.Update
                        : NotificationTypeEnum.Insert
                    : Data?.Element(DeletedTag) != null
        ...
        /// {3} - schema name.
        /// </summary>
        private const string SqlFormatInstallServiceBrokerNotification = @"
        ...
        public SqlDependencyEx(
            ...
            NotificationTypeEnum listenerType =
                NotificationTypeEnum.Insert | NotificationTypeEnum.Update | NotificationTypeEnum.Delete,
        ...
        public static int[] GetDependencyDbIdentities(string connectionString, string database)
        {
            ...
            return
                result.Select(p => int.TryParse(p, out var temp) ? temp : -1)
                    .Where(p => p != -1)
                    .ToArray();
        }
        ...
        private string GetInstallNotificationProcedureScript()
        {
            string installServiceBrokerNotificationScript = string.Format(
                SqlFormatInstallServiceBrokerNotification,
                DatabaseName,
                ConversationQueueName,
                ConversationServiceName,
                SchemaName);
        ...
        private string GetTriggerTypeByListenerType()
        {
            ...
            if (NotificationType.HasFlag(NotificationTypeEnum.Insert))
                result.Append("INSERT");
            if (NotificationType.HasFlag(NotificationTypeEnum.Update))
                result.Append(result.Length == 0 ? "UPDATE" : ", UPDATE");
            if (NotificationType.HasFlag(NotificationTypeEnum.Delete))
        ...
        {
            var evt = TableChanged;
            evt?.Invoke(this, new TableChangedEventArgs(message));
        }

        private void OnNotificationProcessStopped()
        {
            var evt = NotificationProcessStopped;
            evt?.BeginInvoke(this, EventArgs.Empty, null, null);
        }
    }
}